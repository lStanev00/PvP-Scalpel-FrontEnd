...
export default function PvPCards({ bracketData, bracketLabel = undefined, specLabel = undefined }) {
    const hasRecord = bracketData?.achieves?.media || bracketData?.record;
    ...
    const hasRating = rawRating != null && (rawRating > -1 || (rawRating === 0 && hasPlayedGames));

    if (!hasRating && !hasRecord) console.info(bracketData)
            ...
            <div className={Style.cardMain}>
                ...
                {hasRating && (
                    <section
                        ...
                        <div className={Style.heroMeta}>
                            ...
                            {ratingName && (
                                <strong className={Style.heroTitle}>{ratingName}</strong>
                            )}
                        </div>
                ...
                {hasRecord && (
                    <section className={`${Style.cardSection} ${Style.recordSection}`}>
                        <div className={Style.recordMediaAndText}>
                            {" "}
                            {bracketData?.achieves?.media && (
                                <img
                            ...
                            <div className={Style.recordCopy}>
                                ...
                                <strong className={Style.recordTitle}>
                                    {bracketData?.achieves?.name}
                                </strong>
                            </div>
                        </div>
