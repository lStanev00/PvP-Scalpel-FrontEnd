...
export function UsernameForm({ currentUsername, setEditUsername }) {
    ...
    const handleUsernameModification = async (e) => {
        ...
        try {
            ...
            if (status == 500)
                return setError(`Internal server error! Please report or open issue on github.`);
            if (status == 201) {
                setUser(req.data);
                setTimeout(async () => {
    ...
    return (
        <>
            <div>
                <form
                    ref={(el) => (refList.current = el)}
                    style={{ marginBottom: `10px` }}
                    onSubmit={async (e) => await handleUsernameModification(e)}>
...
export function EmailForm({ email, setEditEmail }) {
    ...
    return (
        <div>
            <form
                ref={(el) => (refList.current = el)}
                onSubmit={async (e) => await handleEmailModification(e)}>
